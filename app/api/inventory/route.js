...
export async function POST(req) {
    const {name, description, id_category, image_path, quantity, price} = await req.json(); // Asegúrate de extraer el cuerpo de la solicitud
    
    try {
        // Llama a la función que se conecta con la base de datos
        await insertItem({name, description, id_category, image_path, quantity, price});
