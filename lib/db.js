...
async function insertItem({name, description, id_category, image_path, quantity, price}) {
    await sql`INSERT INTO products (name, description, id_category, image_path, quantity, price)
              VALUES (${name}, ${description}, ${id_category}::int, ${image_path}, ${quantity}::int, ${price}::int)`;
}
